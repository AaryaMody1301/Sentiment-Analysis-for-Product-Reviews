...
st.set_page_config(page_title="Reliable Inference", layout="wide")
...
with source_tab:
    ...
    if frame is not None and not frame.empty:
        ...
        if len(columns) < 2:
            st.error("Training requires separate review-text and sentiment-label columns.")
        else:
...
with bundle_tab:
    ...
    st.subheader("Inspected skops artifacts")
    if not safe_saved:
        st.info("No `.inference.skops` bundles were found in models/.")
    else:
        selected_safe = st.selectbox(
            "Local skops bundle", safe_saved, format_func=lambda path: path.name
        )
        try:
...
if bundle is not None:
    ...
    batch_reviews = st.text_area("Batch reviews (one review per line)", key="reliable_batch_reviews")
    ...
    if st.button("Save inspected skops bundle", type="primary"):
        try:
